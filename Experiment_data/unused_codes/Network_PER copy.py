...
class ReplayBuffer():
    ...
    def sample_buffer(self, batch_size, alpha, exp_no):
        ...
        with open(f'learn_data_20221206({exp_no}).csv', 'a', encoding='utf-8', newline='') as f: # 샘플링된 데이터 기록
            wr = csv.writer(f)
            for i in batch[:10]:
...
class Agent(): #신경망 학습을 관장하는 클래스
    def __init__(self, lr, gamma, n_actions, epsilon, batch_size, input_dims, per_on,
        eps_dec = 1e-4, eps_end = 0.01, mem_size = 500000, fc1_dims=64,
        fc2_dims=64, fc3_dims=32, replace = 100):
